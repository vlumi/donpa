...
struct FriendDetailView: View {
    ...
    init(friend: Friend, friends: FriendsStore) {
        ...
        _groupsText = State(initialValue: friend.groups.joined(separator: ", "))
    }
    ...
    @ViewBuilder private var content: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("Groups (comma-separated)", bundle: .module)
                    .font(.caption).foregroundStyle(.secondary)
                TextField(text: $groupsText) {
                    Text("e.g. work, family", bundle: .module)
                }
                .textFieldStyle(.roundedBorder)
                .onChangeCompat(of: groupsText) {
                    friends.setGroups(parseGroups($0), for: friend.publicKey)
                }
            }
    ...
    /// Split the comma-separated field into trimmed, de-duplicated, non-empty tags.
    private func parseGroups(_ text: String) -> [String] {
        var seen = Set<String>()
