...
struct CounterReadout: View {
    ...
    /// Whole-second timer: zero-padded 3-digit (e.g. `047`) under 1000s, then
    /// `m:ss`, rolling into `h:mm:ss` past an hour so a marathon board keeps
    /// counting instead of sticking (the recorded time was always exact).
